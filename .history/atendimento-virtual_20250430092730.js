...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function processarAgendamento(mensagem) {
        ...
        const palavrasAgendamento = ["agendar", "marcar", "horario", "horário", "reservar", "quero cortar", "quero fazer", 
                                   "agendar horario", "agendar ", "marcar horario", "marcar horário", "vaga", 
                                   "disponibilidade", "tem horario", "tem horário"];
        
        if (contemPalavraChave(msgLower, palavrasAgendamento) || 
            (contemPalavraChave(msgLower, ["quero", "gostaria", "preciso", "queria"]) && 
             contemPalavraChave(msgLower, ["corte", "cortar", "cabelo", "barba"]))) {
            
            // Identificar serviço
            let servico = "um serviço";
            
            // Verificar cortes
            if (contemPalavraChave(msgLower, ["fade", "americano", "feid", "feidi"])) {
                servico = "Fade Americano";
            } else if (contemPalavraChave(msgLower, ["social", "classico", "clássico", "tradicional"])) {
                servico = "Corte Social";
            } else if (contemPalavraChave(msgLower, ["navalhado", "navalha", "degrade", "degradê", "sombreado"])) {
                servico = "Navalhado com Degradê";
            } else if (contemPalavraChave(msgLower, ["infantil", "criança", "crianca", "menino", "kids"])) {
                servico = "Corte Infantil";
            } else if (contemPalavraChave(msgLower, ["nudred", "nudread", "afro"])) {
                servico = "Nudred Infantil";
            } else if (contemPalavraChave(msgLower, ["corte", "cortar", "cabelo", "cabeleireiro"])) {
                servico = "Corte de Cabelo";
            }
            // Verificar barba
            else if (contemPalavraChave(msgLower, ["design", "desenho", "desing"]) && 
                     contemPalavraChave(msgLower, ["barba", "barbear"])) {
                servico = "Design de Barba";
            } else if (contemPalavraChave(msgLower, ["acabamento", "finalização", "finalizacao", "aparar"]) && 
                       contemPalavraChave(msgLower, ["barba", "barbear"])) {
                servico = "Acabamento de Barba";
            } else if (contemPalavraChave(msgLower, ["barba completa", "barba cheia", "tratamento barba"])) {
                servico = "Barba Completa";
            } else if (contemPalavraChave(msgLower, ["barba", "barbear", "fazer a barba"])) {
                servico = "Barba";
            }
            // Verificar combo
            else if ((contemPalavraChave(msgLower, ["cabelo", "corte"]) && 
                      contemPalavraChave(msgLower, ["barba", "barbear"])) || 
                     contemPalavraChave(msgLower, ["combo", "completo", "pacote", "dois", "2 serviços"])) {
            ...
            if (contemPalavraChave(msgLower, ["amanhã", "amanha", "dia seguinte", "proximo dia"])) {
                dia = "amanhã";
            } else if (contemPalavraChave(msgLower, ["segunda", "segunda-feira", "segunda feira"])) {
                dia = "segunda-feira";
            } else if (contemPalavraChave(msgLower, ["terça", "terca", "terça-feira", "terca-feira", "terça feira"])) {
                dia = "terça-feira";
            } else if (contemPalavraChave(msgLower, ["quarta", "quarta-feira", "quarta feira"])) {
                dia = "quarta-feira";
            } else if (contemPalavraChave(msgLower, ["quinta", "quinta-feira", "quinta feira"])) {
                dia = "quinta-feira";
            } else if (contemPalavraChave(msgLower, ["sexta", "sexta-feira", "sexta feira"])) {
                dia = "sexta-feira";
            } else if (contemPalavraChave(msgLower, ["sabado", "sábado"])) {
                dia = "sábado";
            } else if (contemPalavraChave(msgLower, ["domingo"])) {
                dia = "domingo";
            }
            ...
            const regexHorario = /(\d{1,2})[:h\s](\d{0,2})|(\d{1,2})\s*(hrs|horas|h$)/i;
    ...
    function simularRespostaAtendente(mensagemUsuario) {
        ...
        setTimeout(() => {
            ...
            if ((contemPalavraChave(msgLower, ["horario", "horário", "hora", "horas", "hrs", "expediente"]) && 
                ...
                contemPalavraChave(msgLower, ["que horas abre", "que horas fecha", "horario de abertura", "horário de abertura", 
                                           ...
                                           "fim de semana", "domingo", "sabado", "sábado"])) {
            ...
            // Preços de cortes
            else if ((contemPalavraChave(msgLower, ["preço", "preco", "valor", "custo", "quanto custa", "quanto é", "quanto e", "preços", "precos"]) && 
                     contemPalavraChave(msgLower, ["corte", "cortar", "cabelo", "cabeleireiro", "fade", "degrade", "degradê", "navalhado"]))) {
            ...
            // Preços de barba
                     ...
                     contemPalavraChave(msgLower, ["barba", "barbear", "aparar barba", "fazer a barba"]))) {
            ...
            // Preços gerais
            else if (contemPalavraChave(msgLower, ["preço", "preco", "valor", "custo", "quanto custa", "quanto é", "quanto e", "preços", "precos", "tabela"])) {
                resposta = listarServicos("todos");
            }
            // Fade
            else if (contemPalavraChave(msgLower, ["fade", "feid", "feidi", "degrade americano", "degradê americano"])) {
            ...
            // Cortes infantis
            else if ((contemPalavraChave(msgLower, ["corte", "cabelo", "cortar"]) && 
                     contemPalavraChave(msgLower, ["infantil", "criança", "crianca", "menino", "garoto", "filho", "kids", "infantis"]))) {
            ...
            // Cortes gerais
            else if (contemPalavraChave(msgLower, ["corte", "cortar", "cabelo", "cabeleireiro", "cortes", "tipos de corte"])) {
                resposta = listarServicos("cortes");
            }
            // Barba
            else if (contemPalavraChave(msgLower, ["barba", "barbear", "aparar barba", "fazer a barba", "barba completa", "design barba"])) {
                resposta = listarServicos("barba");
            }
            // Combos e promoções
            else if (contemPalavraChave(msgLower, ["combo", "combos", "promoção", "promocao", "desconto", "pacote", "oferta", "especial"])) {
                resposta = '<strong>Combos e Promoções:</strong><br>• Corte + Barba Simples: R$ 65,00 (55 min)<br>• <b>Corte + Barba Completa<b>: R$ 70,00 (60 min)<br>• PROMOÇÃO DE MARÇO: 20% OFF em qualquer serviço às quartas-feiras!<br><br>Deseja agendar algum desses combos?';
            }
            // Endereço e localização
            else if (contemPalavraChave(msgLower, ["endereço", "endereco", "localização", "localizacao", "onde fica", "como chegar", "lugar", "local", "bairro", "rua"])) {
            ...
            // WhatsApp
            else if (contemPalavraChave(msgLower, ["whatsapp", "whats", "wpp", "zap", "zapzap"])) {
                const whatsappLink = `https://wa.me/5511999999999?text=${encodeURIComponent("Olá! Gostaria de informações sobre a Barbearia do Di.")}`;
                resposta = `Você pode nos contatar pelo WhatsApp: ${dadosContato.telefone}<br><br><a href="${whatsappLink}" target="_blank" class="text-success">Iniciar conversa no WhatsApp</a>`;
            }
            // Formas de pagamento
            else if (contemPalavraChave(msgLower, ["pagamento", "pagar", "cartão", "cartao", "credito", "crédito", "debito", "débito", "pix", "dinheiro", "transferência", "transferencia"])) {
            ...
            else if (contemPalavraChave(msgLower, ["especialidades", "especialidade", "serviços", "servicos", "fazem", "oferecem", "tem", "trabalham"])) {
            ...
            // Estacionamento
            else if (contemPalavraChave(msgLower, ["estacionamento", "parar", "carro", "moto", "veiculo", "veículo", "estacionar"])) {
                resposta = 'Sim, temos estacionamento próprio e gratuito para nossos clientes.';
            }
            // Cancelamento
            else if (contemPalavraChave(msgLower, ["cancelar", "desmarcar", "remarcar", "reagendar", "adiar"])) {
                resposta = 'Para cancelar ou remarcar um agendamento, por favor, informe seu nome completo e a data/horário agendado. Pedimos que cancele com pelo menos 2 horas de antecedência.';
                       ...
                       // Agradecimento
                       else if (contemPalavraChave(msgLower, ["obrigado", "obrigada", "valeu", "agradeço", "agradecido", "grato", "grata", "thanks", "vlw", "brigado", "brigada", "obg"])) {
                        resposta = 'Por nada! Estamos sempre à disposição. Esperamos vê-lo em breve na Barbearia do DI!';
                    }
                    // Saudações
                    else if (contemPalavraChave(msgLower, ["olá", "ola", "oi", "bom dia", "boa tarde", "boa noite", "e ai", "e aí", "eai", "hello", "hi", "tudo bem", "tudo bom"])) {
                    ...
                    // Contato
                    else if (contemPalavraChave(msgLower, ["contato", "telefone", "email", "e-mail", "fone", "ligar", "numero", "número", "contatos"])) {
                        resposta = `<strong>Contatos:</strong><br>
                        • Telefone/WhatsApp: ${dadosContato.telefone}<br>
                        • E-mail: ${dadosContato.email}<br>
            ...
            // Adicionar estilos CSS para os botões de agendamento
